...
const OS = (() => {
  ...
  const showDesktop = () => {
    ...
    document.getElementById('desktop').addEventListener('contextmenu', e => {
      ...
      showContextMenu(e.clientX, e.clientY, [
        ...
        { icon:'🛒', label:'NormShop',          action: ()=>apps.open('shop') },
        { sep:true },
        { icon:'📱', label:'NormTok',           action: ()=>apps.open('normtok') },
        { icon:'👤', label:'My Profile',        action: ()=>apps.open('profile') },
        { icon:'👥', label:'Friends',           action: ()=>apps.open('friends') },
        { icon:'🏦', label:'NormBank Loans',    action: ()=>apps.open('loans') },
        ...
        { icon:'📧', label:'NormMail',          action: ()=>apps.open('mail') },
        { icon:'🏪', label:'NormHub',           action: ()=>apps.open('appstore') },
  ...
  const buildDesktopIcons = () => {
    ...
    const iconApps = [
      'terminal','files','browser','notepad','texteditor',
      'mail','news','sysmon','snake','chat','paint',
      'normsheet','calculator','clock','music','calendar',
      'imagedrop','stocks','leaderboard','shop',
      // New apps
      'normtok','profile','friends','loans','miner',
      'casino','mail','settings','appstore',
    ];
