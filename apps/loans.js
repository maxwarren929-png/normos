...
const LoansApp = {
  create() {
    ...
    const renderHackableBank = (b) => {
      ...
      return `<div class="ml-hackable" style="--bcolor:${b.color}">
        ...
        <div class="ml-two-col">
          <div class="ml-panel">
            <div class="ml-panel-title">💰 Deposit / Withdraw</div>

            <div class="ml-field-group">
              <label class="ml-field-label">Deposit amount</label>
              <div class="ml-field-row">
                <input type="number" id="ml-dep-${iid}" class="ml-input" placeholder="Enter amount..." min="1"/>
              </div>
              ...
              <button class="ml-btn ml-btn-color" id="ml-dep-btn-${iid}" style="--bc:${b.color};margin-top:0.25rem">💵 Deposit</button>
            </div>

            <div class="ml-field-group" style="margin-top:1rem">
              <label class="ml-field-label">Withdraw <span style="color:var(--text3);font-size:0.6rem">(${b.fee}% fee)</span></label>
              <div class="ml-field-row">
                <input type="number" id="ml-wdw-${iid}" class="ml-input" placeholder="Enter amount..." min="1"/>
              </div>
              <div class="ml-quick-row">
              ...
              <button class="ml-btn ml-btn-dark" id="ml-wdw-btn-${iid}" style="margin-top:0.25rem">🏧 Withdraw</button>
            </div>

            <div class="ml-msg" id="ml-msg-${iid}" style="margin-top:0.5rem;min-height:1.2em"></div>

            <div class="ml-lb-title" style="margin-top:0.75rem">🏆 Top Depositors</div>
    ...
    if (typeof Network !== 'undefined') {
      ...
      const onMbD = (msg) => { if(msg.banks)multiBankData={...multiBankData,...msg.banks}; rerender(); };
      const onMbU = (msg) => {
      ...
      const onMbI = (msg) => { if(typeof OS!=='undefined')OS.notify(HACKABLE_BANKS[msg.bankId]?.icon||'🏦',msg.bankName,`+$${(msg.amount||0).toFixed(2)} interest!`); };
      const onMbH = (msg) => {
      ...
      const onMbHacked = (msg) => { if(typeof OS!=='undefined')OS.notify('💀','HACKED!',`${msg.by} stole $${(msg.lost||0).toFixed(2)} from ${msg.bakName||msg.bankName}!`); };
      const onMbErr = (msg) => {
    ...
    if (!document.getElementById('mbl-styles')) {
      ...
      st.textContent = `.bank-wrap{height:100%;overflow:hidden;background:var(--bg1)}.mbl-layout{display:flex;height:100%;overflow:hidden}.mbl-sidebar{width:230px;flex-shrink:0;background:var(--bg2);border-right:1px solid var(--border);display:flex;flex-direction:column;overflow:hidden}.mbl-sidebar-top{padding:.85rem 1rem 0;flex-shrink:0}.mbl-title{font-size:.85rem;font-weight:700;margin-bottom:.65rem;color:var(--text1)}.mbl-cash-card{background:var(--bg1);border:1px solid var(--border);border-radius:8px;padding:.75rem 1rem;margin-bottom:.75rem}.mbl-cash-label{font-size:.62rem;color:var(--text3);margin-bottom:3px}.mbl-cash-val{font-size:1.25rem;font-weight:700;color:#4ade80;font-family:var(--font-mono)}.mbl-cash-sub{font-size:.6rem;color:var(--text3);margin-top:3px}.mbl-nav{flex:1;overflow-y:auto;padding:.3rem .5rem;display:flex;flex-direction:column;gap:.25rem}.mbl-nav-item{display:flex;align-items:center;gap:.55rem;padding:.65rem .7rem;border-radius:7px;cursor:pointer;border:1px solid transparent;transition:background .12s,border-color .12s}.mbl-nav-item:hover{background:var(--bg3)}.mbl-nav-item.active{background:var(--bg3);border-color:color-mix(in srgb,var(--btc,var(--accent)) 40%,transparent)}.mbl-nav-icon{font-size:1.3rem;flex-shrink:0}.mbl-nav-info{flex:1;min-width:0}.mbl-nav-name{font-size:.74rem;font-weight:600;color:var(--text1);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.mbl-nav-sub{font-size:.59rem;color:var(--text3);margin-top:2px}.mbl-nav-dep{font-size:.67rem;font-family:var(--font-mono);flex-shrink:0}.mbl-nav-badge{font-size:.5rem;background:#f87171;color:#000;padding:.1rem .3rem;border-radius:3px;font-weight:700;flex-shrink:0}.mbl-global{padding:.6rem 1rem;border-top:1px solid var(--border);flex-shrink:0}.mbl-global-title{font-size:.58rem;color:var(--text3);text-transform:uppercase;letter-spacing:.04em;margin-bottom:.35rem}.mbl-global-row{display:flex;justify-content:space-between;font-size:.63rem;color:var(--text2);padding:.12rem 0}.mbl-main{flex:1;overflow-y:auto;min-width:0}.ml-loans{padding:1.2rem;display:flex;flex-direction:column;gap:.9rem;max-width:600px;margin:0 auto}.ml-section{background:var(--bg2);border:1px solid var(--border);border-radius:10px;padding:1.1rem}.ml-active-loan{border-color:#f8717166}.ml-section-head{font-size:.84rem;font-weight:700;margin-bottom:.8rem}.ml-credit-row{display:flex;align-items:center;gap:.8rem;margin-bottom:.55rem}.ml-score-val{font-size:1.55rem;font-weight:800;font-family:var(--font-mono);line-height:1}.ml-score-tier{font-size:.74rem;font-weight:600;margin-top:3px}.ml-score-bar-wrap{flex:1;background:var(--bg1);border-radius:3px;height:6px;overflow:hidden}.ml-score-bar{height:100%;border-radius:3px;transition:width .4s}.ml-hint{font-size:.66rem;color:var(--text3);line-height:1.5}.ml-loan-grid{display:grid;grid-template-columns:1fr 1fr;gap:.55rem;margin-bottom:.7rem}.ml-lstat{background:var(--bg1);border:1px solid var(--border);border-radius:7px;padding:.6rem .75rem}.ml-ls-l{font-size:.59rem;color:var(--text3);margin-bottom:3px}.ml-ls-v{font-size:.9rem;font-weight:700;font-family:var(--font-mono)}.ml-bar-wrap{background:var(--bg1);border-radius:3px;height:5px;overflow:hidden;margin-bottom:.7rem}.bank-loan-bar{height:100%;border-radius:3px;transition:width 1s linear}.ml-loan-offer{background:var(--bg1);border:1px solid var(--border);border-radius:8px;padding:.85rem;margin-bottom:.7rem;display:flex;flex-direction:column;gap:.4rem}.ml-offer-row{display:flex;justify-content:space-between;align-items:center;font-size:.74rem}.ml-offer-label{color:var(--text3)}.ml-offer-val{font-family:var(--font-mono);font-weight:600}.ml-btn{border:none;border-radius:7px;padding:.6rem 1rem;font-size:.76rem;cursor:pointer;font-weight:700;transition:opacity .12s;width:100%}.ml-btn:hover:not([disabled]){opacity:.85}.ml-btn[disabled]{opacity:.4;cursor:not-allowed}.ml-btn-green{background:#4ade80;color:#000}.ml-btn-accent{background:var(--accent);color:#fff}.ml-btn-dark{background:#374151;color:#e5e7eb}.ml-btn-color{background:var(--bc,var(--accent));color:#fff}.ml-btn-disabled{background:var(--bg3);color:var(--text3)}.ml-tier-list{display:flex;flex-direction:column;gap:.35rem}.ml-tier-row{display:flex;align-items:center;gap:.6rem;padding:.5rem .7rem;border-radius:6px;font-size:.7rem;background:var(--bg1);border:1px solid var(--border)}.ml-tier-row.unlocked{border-color:rgba(74,222,128,.25)}.ml-tier-row.locked{opacity:.38}.ml-tier-name{font-weight:700;min-width:66px}.ml-tier-amt{font-family:var(--font-mono);font-weight:700;min-width:76px}.ml-tier-detail{color:var(--text3);font-size:.6rem;margin-left:auto}.ml-hackable{padding:1.2rem;display:flex;flex-direction:column;gap:.9rem}.ml-hack-header{display:flex;align-items:center;gap:.85rem;padding-bottom:.9rem;border-bottom:2px solid var(--bcolor,var(--accent))}.ml-hack-icon{font-size:2.4rem;flex-shrink:0}.ml-hack-title-block{flex:1}.ml-hack-name{font-size:1.05rem;font-weight:700}.ml-hack-tagline{font-size:.63rem;color:var(--text3);font-style:italic;margin-top:3px}.ml-hack-sec{font-size:.9rem;flex-shrink:0}.ml-stats-row{display:flex;gap:.7rem}.ml-stat-card{flex:1;background:var(--bg2);border:1px solid var(--border);border-radius:8px;padding:.75rem;text-align:center}.ml-stat-label{font-size:.59rem;color:var(--text3);margin-bottom:4px}.ml-stat-val{font-size:1rem;font-weight:700;font-family:var(--font-mono)}.ml-stat-sub{font-size:.59rem;color:var(--text3);margin-top:4px}.ml-two-col{display:grid;grid-template-columns:1fr 1fr;gap:.85rem}.ml-panel{background:var(--bg2);border:1px solid var(--border);border-radius:9px;padding:1rem;display:flex;flex-direction:column;gap:.5rem;min-width:0}.ml-panel-title{font-size:.78rem;font-weight:700;margin-bottom:.45rem;color:var(--text1)}.ml-field-group{display:flex;flex-direction:column;gap:.32rem}.ml-field-label{font-size:.62rem;color:var(--text3);font-weight:600}.ml-field-row{display:flex;gap:.4rem}.ml-input{width:100%;background:var(--bg1);border:1.5px solid var(--border);border-radius:6px;color:var(--text1);font-size:.82rem;padding:.5rem .65rem;outline:none;font-family:var(--font-mono)}.ml-input:focus{border-color:var(--bcolor,var(--accent))}.ml-quick-row{display:flex;gap:.25rem;flex-wrap:wrap}.ml-qbtn{font-size:.62rem;padding:.25rem .48rem;background:var(--bg1);border:1px solid var(--border);border-radius:4px;cursor:pointer;color:var(--text2);transition:border-color .1s,color .1s}.ml-qbtn:hover{border-color:var(--bcolor,var(--accent));color:var(--bcolor,var(--accent))}.ml-msg{font-size:.67rem;min-height:1em;font-family:var(--font-mono)}.ml-lb-title{font-size:.72rem;font-weight:700;padding-top:.6rem;border-top:1px solid var(--border);margin-top:.35rem}.ml-lb-list{display:flex;flex-direction:column;gap:.18rem}.ml-lb-empty{font-size:.63rem;color:var(--text3);padding:.35rem 0}.ml-lb-row{display:flex;align-items:center;gap:.35rem;font-size:.65rem;padding:.2rem 0}.ml-lb-rank{width:1.3rem;flex-shrink:0}.ml-lb-name{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.ml-lb-amt{font-family:var(--font-mono);font-weight:600;flex-shrink:0}.ml-lb-pct{font-size:.57rem;color:var(--text3);width:2.8rem;text-align:right;flex-shrink:0}.ml-hack-info-grid{background:var(--bg1);border:1px solid var(--border);border-radius:7px;padding:.65rem .8rem;display:flex;flex-direction:column;gap:.3rem}.ml-hig-row{display:flex;justify-content:space-between;font-size:.64rem;color:var(--text2);padding:.05rem 0}.ml-hack-rules{font-size:.61rem;color:var(--text3);display:flex;flex-direction:column;gap:.22rem;line-height:1.6}.ml-hack-btn{width:100%;padding:.8rem;background:linear-gradient(135deg,#7f1d1d,#dc2626);color:#fff;border:1px solid #f87171;border-radius:8px;cursor:pointer;font-size:.82rem;font-weight:700;letter-spacing:1px;transition:transform .1s,box-shadow .1s;margin-top:auto}.ml-hack-btn:hover:not([disabled]){transform:scale(1.02);box-shadow:0 4px 18px rgba(220,38,38,.4)}.ml-hack-btn[disabled]{opacity:.4;cursor:not-allowed}.ml-cd-box{text-align:center;font-size:.74rem;color:var(--text3);padding:.8rem;background:var(--bg1);border-radius:7px;border:1px solid var(--border);margin-top:auto}.ml-cd-num{color:#f87171;font-weight:700;font-family:var(--font-mono)}`;
