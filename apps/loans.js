  ...
  create() {
    ...
    const render = () => {
        ...
        <div class="bank-layout">
          ...
          <div class="bank-section">
            ...
            <div style="font-size:0.75rem;color:var(--text2);margin-bottom:8px;">
              Your tier <strong style="color:${tier.color}">${esc(tier.name)}</strong> — fixed loan amount:
              <strong style="color:${tier.color}">$${tier.loanCap.toLocaleString()}</strong>
            </div>
            <div class="bank-row">
              <input class="bank-input" id="bank-loan-amt-${iid}" type="number" value="${tier.loanCap}" min="${tier.loanCap}" max="${tier.loanCap}" readonly style="opacity:0.7;" />
              <button class="bank-btn accent" id="bank-loan-btn-${iid}">Request $${tier.loanCap.toLocaleString()}</button>
            </div>
            <div class="bank-tier-ladder">
              ${CREDIT_TIERS.map(t=>`
                <div class="bank-tier-item ${(s.creditScore||0)>=t.minScore?'unlocked':'locked'}">
                  ...
                  <span>exact: $${t.loanCap.toLocaleString()}</span>
                  <span style="margin-left:auto;color:var(--text3)">${t.minScore}+ pts</span>
                </div>`).join('')}
            </div>
            <div class="bank-hint">Each tier unlocks a fixed loan size. Repay on time: <strong style="color:#4ade80">+50 score</strong>. Late: <strong style="color:var(--red)">-100</strong>. Default: <strong style="color:var(--red)">-200 + lose everything</strong>.</div>
          </div>`}
        </div>`;
      ...
      wrap.querySelector(`#bank-loan-btn-${iid}`)?.addEventListener('click',()=>{
        const amt=tier.loanCap;
        if(!confirm(`Borrow exactly $${fmt(amt)} (your ${tier.name} tier loan)?\n\nMissing the deadline = EVERYTHING is seized.`))return;
