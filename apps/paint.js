...
const PaintApp = {
  create() {
    ...
    wrap.querySelector('#paint-save').addEventListener('click', () => {
      const name = (prompt('Save as:', 'drawing.png') || 'drawing.png').replace(/[<>:"\/\\|?*]/g,'_');
      ...
      if (typeof FS !== 'undefined') {
        ...
        // Fallback: store in localStorage under normos_files
        try {
          const files = JSON.parse(localStorage.getItem('normos_myfiles') || '{}');
          files[name] = { name, dataUrl, type:'image/png', savedAt: Date.now() };
          localStorage.setItem('normos_myfiles', JSON.stringify(files));
        } catch {}
      }
      if (typeof OS !== 'undefined') OS.notify('💾', 'NormPaint', `Saved: ${name} → My Files`);
    });
