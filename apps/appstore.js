...
const AppStoreApp = {
  create() {
    ...
    const APPS = [
      ...
      { id:'bank',        name:'NormBank Central', icon:'🏦', cat:'finance', price:0,    rating:4.5, reviews:2800, desc:'Deposits, loans with credit scoring, passive interest, and borrowing up to $250k. One bank to rule them all.', publisher:'NormBank Corp', builtIn:true,  appId:'bank' },
      { id:'social',      name:'Social Hub',       icon:'👥', cat:'social',  price:0,    rating:4.6, reviews:3400, desc:'Friends, profile, and direct messages with file attachments — all in one.', publisher:'NormOS Core', builtIn:true,  appId:'social' },
      { id:'voidtracker', name:'VOID Tracker Pro', icon:'🌑', cat:'finance', price:500,  rating:3.2, reviews:891,  desc:'Real-time VoidToken alerts. Crash predictions (60% wrong).', publisher:'VoidCorp Analytics', builtIn:false, appId:'stocks' },
      { id:'normfirewall',name:'NormFirewall',     icon:'🔥', cat:'system',  price:10000,rating:4.9, reviews:1847, desc:'Blocks all incoming hacks for 5 minutes. One-time use per activation.', publisher:'NormSec Inc', builtIn:false, purchasable:true, appId:null },
      { id:'normsnake',   name:'Snake (Premium)',  icon:'🐍', cat:'games',   price:500,  rating:4.7, reviews:2103, desc:'Full Snake game with high scores, speed levels, and neon skins.', publisher:'NormGame Studios', builtIn:false, appId:'snake' },
      { id:'cryptopet',   name:'CryptoPet',        icon:'🐱', cat:'games',   price:200,  rating:4.6, reviews:4400, desc:'Desktop tamagotchi. Feed it NormCoin. Neglect it and it judges you.', publisher:'PetDev Corp', builtIn:false, appId:'miner' },
      { id:'normdate',    name:'NormDate',         icon:'💘', cat:'social',  price:0,    rating:2.1, reviews:43,   desc:'Dating app for NormOS users. Matched by net worth.', publisher:'NormDate LLC', builtIn:false, appId:'leaderboard' },
      { id:'normdocs',    name:'NormDocs',         icon:'📄', cat:'system',  price:150,  rating:3.8, reviews:234,  desc:'Word processor with .normdoc format.', publisher:'NormSoft', builtIn:false, appId:'texteditor' },
    ];
    ...
    wrap.innerHTML = `
      <div class="as-layout">
        <div class="as-sidebar">
          <div class="as-logo">🏪 NormHub & Shop</div>
    ...
    const renderMain = () => {
      ...
      mainEl.querySelectorAll('.as-buy-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          // Special: Firewall activates on purchase
          if (appId === 'normfirewall') {
            installs[appId] = false; // reusable — don't mark as permanently installed
            saveInstalls(installs);
            const expiresAt = Date.now() + 5 * 60 * 1000;
            try { localStorage.setItem('normos_firewall_until', String(expiresAt)); } catch {}
            if (typeof OS !== 'undefined') OS.notify('🔥', 'NormFirewall', 'Active! You are protected from all hacks for 5 minutes.');
            renderMain();
            return;
          }
          if (appDef.appId && typeof OS !== 'undefined') OS.apps.open(appDef.appId);
          renderMain();
        });
      });
    };
